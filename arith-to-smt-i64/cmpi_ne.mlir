...
"builtin.module"() ({
  "func.func"() ({
  ^0(%x : i64, %y : i64):
    %r = "arith.cmpi"(%x, %y) {"predicate" = 1 : i64} : (i64, i64) -> i1
    "func.return"(%r) : (i1) -> ()
  }) {"sym_name" = "test", "function_type" = (i64, i64) -> i1, "sym_visibility" = "private"} : () -> ()
}) : () -> ()
...
// CHECK-NEXT:      %7 = "smt.distinct"(%2, %4) : (!smt.bv.bv<32>, !smt.bv.bv<32>) -> !smt.bool
...
// CHECK-NEXT:    }) {"fun_name" = "test"} : () -> ((!smt.utils.pair<!smt.bv.bv<32>, !smt.bool>, !smt.utils.pair<!smt.bv.bv<32>, !smt.bool>, !effect.state) -> (!smt.utils.pair<!smt.bv.bv<1>, !smt.bool>, !effect.state))
// CHECK-NEXT:  }
